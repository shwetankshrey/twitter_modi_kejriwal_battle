# Sends a GET request to the Twitter API and stores the JSON documents to a collection.
...
# Two queries as max query size is 140 characters
query = "#modi OR #kejriwal OR #narendra OR #arvind OR #narendramodi OR #arvindkejriwal OR #namo OR #aap OR #bjp"
query1 = "#kejri OR #modigovernment OR #aapgovernment OR #pradhanmantri OR #pmmodi OR #modiscaredofarvind OR #pm OR #delhicm"

maxTweets = 20000 # 20000 to be queried to get atleast 10000 original tweets
tpq = 100 # Tweets per query
...
while count < maxTweets: # To keep the extraction running even if exception is raised.
    try:
        ...
        if not new_tweets:
            ...
            break
        for tweet in new_tweets: # Here we go...
            ...
            try:
                ...
                coll.insert_one(jsonx) # And gone gone gone...
            except pymongo.errors.DuplicateKeyError as e: # Except if it's already there, in which case not gone gone gone...
        ...
        if count > 10000: # To switch to the other query
            max_id = -1
            query = query1
